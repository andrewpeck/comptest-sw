...
//void Mux::halfstripMaptoMux (uint32_t halfstrips)
//{
//
//}

int Mux::amp (PulseLevel_t level)
{
    if (level==HIGH)
        return 3;
    else if (level==MED)
...
void Mux::writeCompoutExpect (int compout_expect)
{
...
void Mux::configStripLH (int strip, struct MuxConfig_t &config)
{
...
void Mux::configStripRH (int strip, struct MuxConfig_t &config)
{
    config.in[strip] = HIGH;
...
void Mux::configAllChannelsOff(struct MuxConfig_t &config)
{
    config.next = OFF;
    config.prev = OFF;
    for (int i=0; i<16; i++)
